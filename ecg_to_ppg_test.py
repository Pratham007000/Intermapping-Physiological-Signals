...
# Import the LSTM model (assumes lstm_ppg_model.py contains the ECGtoPPG_LSTM class)
...
# Generate synthetic PPG target from ECG
def generate_ppg_from_ecg(ecg_data, delay=100, pulse_width=80):
    """Generate synthetic PPG signal from ECG for testing purposes"""
...
def main():
    ...
    # Use only the test split (last 15% of data)
    ...
    model_path = "model_checkpoints/best_lstm_model_original.pth"
    ...
    actual_ppg = Y_test[:100, :, 0].reshape(-1)[:500]  # Include actual PPG for comparison
    
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    
    plt.figure(figsize=(14, 8))
    plt.plot(actual_ecg, label="ECG Signal", color='blue', linewidth=2, alpha=0.8)
    plt.plot(actual_ppg, label="Actual PPG", color='green', linewidth=2, alpha=0.8)
    plt.plot(predicted_ppg, label="Predicted PPG", color='red', linestyle='--', linewidth=2)
    
    plt.title("ECG Signal, Actual PPG, and Predicted PPG (First 500 Timepoints)", fontsize=16, fontweight='bold')
    ...
    # Save the figure
    output_path = f"ecg_actual_predicted_ppg_results_{timestamp}.png"
