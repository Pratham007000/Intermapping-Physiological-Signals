...
def generate_ppg_from_ecg(ecg_data, delay=200, pulse_width=150, cache_file="ppg_signal_151604.npy"):
...
def create_sequences(x, y, seq_len=200, cache_x="test_sequences_X_151604.npy", cache_y="test_sequences_Y_151604.npy"):
    if os.path.exists(cache_x) and os.path.exists(cache_y):
        print(f"Loading cached test sequences from {cache_x} and {cache_y}")
        X = np.load(cache_x)
        Y = np.load(cache_y)
        if X.shape[1] == seq_len and Y.shape[1] == seq_len:
            return X, Y
        else:
...
print("Loading data...")
ecg = load_ecg_data()
print("Generating or loading cached PPG target from ECG...")
ppg = generate_ppg_from_ecg(ecg, delay=200, pulse_width=150)
...
# Create sequences
print("Creating or loading cached sequences...")
X, Y = create_sequences(ecg, ppg, seq_len=200)
X = X[:, :, np.newaxis]
Y = Y[:, :, np.newaxis]

# Test split (use the last 15% of data)
