...
def robust_normalize(signal, percentile_clip=99.9):
...
ecg_processed = wavelet_denoise(ecg, wavelet='db8', level=5)
scg_processed = wavelet_denoise(scg, wavelet='db8', level=5)
ecg_processed = filter_signal(ecg_processed, fs=FS, lowcut=0.5, highcut=50.0)
scg_processed = filter_signal(scg_processed, fs=FS, lowcut=0.5, highcut=30.0)
ecg_processed = robust_normalize(ecg_processed)
scg_processed = robust_normalize(scg_processed)
...
# Find R-peaks in ECG to estimate heart rate
