...
void setup_ble_service() {
    ...
    heartRateCharacteristic.writeValue(60.0);
...
void send_ble_data(float ecg_sample, float ppg_sample) {
    if (BLE.connected()) {
        // Send ECG value
        ecgCharacteristic.writeValue(ecg_sample);
        
        // Send PPG value
        ppgCharacteristic.writeValue(ppg_sample);
        ...
        if (ble_counter % 10 == 0) { // Send heart rate every 10 samples
            float heart_rate = tinyml_model.get_heart_rate_estimate();
            heartRateCharacteristic.writeValue(heart_rate);
        }
        ble_counter++;
    }
}
