...
# Load ECG data (limited to last 10000 samples)
def load_ecg_data(max_samples=10000):
    try:
        data = pd.read_csv("ecg_data_20250701_172937.csv")
        ecg = data["ECG Amplitude"].values[-max_samples:]  # Take last 10000 samples
...
# Generate synthetic PPG or load from cache
def generate_ppg_from_ecg(ecg_data, delay=100, pulse_width=80, cache_file="ppg_signal_10000.npy"):
    ...
    # Save PPG signal to cache
    np.save(cache_file, ppg_signal)
    print(f"Saved PPG signal to {cache_file}")
    return ppg_signal

# Create sequences or load from cache
def create_sequences(x, y, seq_len=100, max_sequences=9000, cache_x="test_sequences_X_9000.npy", cache_y="test_sequences_Y_9000.npy"):
...
# Load ECG and generate/load PPG
print("Loading data...")
ecg = load_ecg_data(max_samples=10000)
print("Generating or loading cached PPG target...")
ppg = generate_ppg_from_ecg(ecg, cache_file="ppg_signal_10000.npy")
...
X, Y = create_sequences(ecg, ppg, seq_len=100, max_sequences=9000)
X = X[:, :, np.newaxis]
Y = Y[:, :, np.newaxis]
...
actual_ecg = X[:100, :, 0].reshape(-1)[:500]
